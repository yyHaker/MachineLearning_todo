...
def trainNB0(trainMatrix, trainCategory):
    """
    朴素贝叶斯分类器训练函数(此处仅处理两类分类问题):从词向量计算概率
    :param trainMatrix: 文档矩阵
    :param trainCategory: 每篇文档类别标签
    :return:
    """
    numTrainDocs = len(trainMatrix)     # 训练样本数
    numWords = len(trainMatrix[0])      # 每篇训练文档的词条数
    pAbusive = sum(trainCategory)/float(numTrainDocs)    # 计算文档类别为1的概率

    # 初始化所有词出现数为1，并将分母初始化为2，避免某一个概率值为0(拉普拉斯修正)
    p0Num = ones(numWords); p1Num = ones(numWords)
    p0Denom = 2.0; p1Denom = 2.0
    for i in range(numTrainDocs):
        if trainCategory[i] == 1:
            p1Num += trainMatrix[i]
            p1Denom += sum(trainMatrix[i])   # ??为什么增加所有词条的计数值，而不是增加样本数
        else:
            p0Num += trainMatrix[i]
...
def textParse(bigString):
    """
    输入文本字符串，输出字符串列表
    :param bigString: 需要切分的文本
    :return:
    """
    import re
    regex = re.compile('\\W*')
    #print(regex)
    listOfTokens = regex.split(bigString)    # 以非单词字符分割
...
def spamTest():
    ...
    for i in range(1, 26):
        ...
        fullText.extend(wordList)
        classList.append(0)
    vocabList = createVocabList(docList)  # 生成词列表

...
由于Boosting算法要求基学习器能对特定的数据分布进行学习，这里采用"重赋权法"实现，
...
#    adaClassify([0,0],weakClassArr)
    ...
    weakClassArr, aggClassEst = adaBoostTrainDS(datArr,labelArr,45)    # 迭代45次就使错误率达到0.1872909699，后面增加迭代次数不会减少错误率
    ...
    m = len(testLabelArr)
    errArr = mat(ones((m, 1)))
    errArr[prediction != mat(testLabelArr).T].sum()/float(m)
